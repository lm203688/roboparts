...
[![Entities](https://img.shields.io/badge/Entities-802-10b981)](./data/)
...
> 覆盖 **20 大品类**（执行器、芯片、传感器、通信协议、机器人AI模型、数据采集设备、接口、平台、大模型、夹爪 / 末端执行器、柔性执行器、仿生机构、减速器、控制器、结构件、连接器、线缆、电源、电路板、一体化关节）的结构化零部件数据集，
...
**直接读数据（不需要任何 key）**
```bash
curl https://roboparts.cc/api/entities.json             # 802 条实体全量
...
1. **802 条实体中，参数口径达到"可跨厂商直接比较"的是 0 条。** 行业普遍不声明测试条件——扭矩不给温升与持续时间、背隙不给加载条件。我们把这个缺陷公开登记（含我方自身数据的同类问题），而不是用漂亮数字掩盖。判据见 [参数口径规范](https://roboparts.cc/robot-joint-parameter-spec)。
2. **机械互换维度（"能不能拧上去"）声明率仅 5.69%。** 适用分母 439 条（总数 802 减去 363 条本身无机械接口的实体）中，仅 15 条完整声明 + 10 条部分声明；其余 414 条如实标注 `not_declared`，我们不做猜测填充。
3. **溯源分布是公开的**：A 406 条（50.6%） · B 246 条（30.7%） · C 150 条（18.7%）。Tier C 为无溯源历史导入，`confidence` 上限 0.30。有原始链接（`source_url`）的实体 515 条（64.2%）——**意味着约 35.8% 的实体目前无法一键回溯到原始文档**。
...
## 数据概览（802 实体 · 20 品类 · 2026-09-24 更新）

| 品类 | 数量 | 高频字段（现算） |
|---|---|---|
| 执行器 (Actuators) | 222 | 类型、厂商、应用 |
...
| 平台 (Platforms) | 43 | 厂商、类型、价格区间、年份、应用 |
...
| 字段 | 覆盖 | 覆盖率 | 说明 |
|---|---|---|---|
| 名称 | 802/802 | 100.0% | 官方型号或通用名称 |
| 厂商 | 575/802 | 71.7% | 采集自官网 / 目录 |
| 原始链接 | 515/802 | 64.2% | 可一键回溯到来源文档 |
| 价格区间 | 213/802 | 26.6% | 参考公开报价 |
| 标准符合性已评估 | 90/802 | 11.2% | ISO / GB / IEC / ROS2 维度 |
...
```json
{
  "meta": {
    ...
    "category_count": 20,
    "category_counts": { "...": 0 },
    "mechanical_interface_declared_rate_pct": 5.69,
