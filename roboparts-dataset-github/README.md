...
[![Entities](https://img.shields.io/badge/Entities-768-10b981)](./data/)
...
> 覆盖 **20 大品类**（执行器、芯片、传感器、通信协议、机器人AI模型、数据采集设备、接口、大模型、平台、柔性执行器、夹爪 / 末端执行器、减速器、仿生机构、控制器、结构件、连接器、线缆、电源、电路板、一体化关节）的结构化零部件数据集，
...
**直接读数据（不需要任何 key）**
```bash
curl https://roboparts.cc/api/entities.json             # 768 条实体全量
...
1. **768 条实体中，参数口径达到"可跨厂商直接比较"的是 0 条。** 行业普遍不声明测试条件——扭矩不给温升与持续时间、背隙不给加载条件。我们把这个缺陷公开登记（含我方自身数据的同类问题），而不是用漂亮数字掩盖。判据见 [参数口径规范](https://roboparts.cc/robot-joint-parameter-spec)。
2. **机械互换维度（"能不能拧上去"）声明率仅 1.52%。** 适用分母 395 条（总数 768 减去 373 条本身无机械接口的实体）中，仅 2 条完整声明 + 4 条部分声明；其余 389 条如实标注 `not_declared`，我们不做猜测填充。
3. **溯源分布是公开的**：A 377 条（49.1%） · B 241 条（31.4%） · C 150 条（19.5%）。Tier C 为无溯源历史导入，`confidence` 上限 0.30。有原始链接（`source_url`）的实体 481 条（62.6%）——**意味着约 37.4% 的实体目前无法一键回溯到原始文档**。
...
## 数据概览（768 实体 · 20 品类 · 2026-09-01 更新）

| 品类 | 数量 | 高频字段（现算） |
|---|---|---|
| 执行器 (Actuators) | 217 | 类型、厂商、应用 |
| 芯片 (Chips) | 108 | 类型、厂商、应用、接口、形态、CPU |
| 传感器 (Sensors) | 90 | 类型、标准合规、厂商、量程 |
...
| 夹爪 / 末端执行器 (Grippers) | 13 | 厂商、类型、应用、规格 |
| 减速器 (Reducers) | 11 | 厂商、类型、应用、减速比、输出扭矩、背隙 |
| 仿生机构 (Bionic Mechanisms) | 9 | 厂商、类型、biomimetic_target、应用、规格、3d_printable |
| 控制器 (Controllers) | 3 | 厂商、类型、应用、规格 |
...
| 字段 | 覆盖 | 覆盖率 | 说明 |
|---|---|---|---|
| 名称 | 768/768 | 100.0% | 官方型号或通用名称 |
| 厂商 | 541/768 | 70.4% | 采集自官网 / 目录 |
| 原始链接 | 481/768 | 62.6% | 可一键回溯到来源文档 |
| 价格区间 | 209/768 | 27.2% | 参考公开报价 |
| 标准符合性已评估 | 88/768 | 11.5% | ISO / GB / IEC / ROS2 维度 |
...
```json
{
  "meta": {
    ...
    "category_count": 20,
    "category_counts": { "...": 0 },
    "mechanical_interface_declared_rate_pct": 1.52,
