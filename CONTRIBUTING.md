...
> **格式层先看规范**：[`docs/part-annotation-spec.md`](./docs/part-annotation-spec.md)
> 是「一条零件实体该长什么样」的唯一权威格式（字段表 + 最小可复制 JSON 模板 + 闸门清单）。
> 本文件讲「什么能收、走哪条通道」，规范讲「收进来长什么样」——两篇配合读，一次就能产出过闸门的实体。

先说结论：**这个项目最缺的不是代码，是一条带出处的孔位数据。**

截至 2026-09-15，全库 798 个实体里机械接口有线索的有 **25 条（5.75%）**——
15 条 `declared` + 10 条 `partial`，分母是 435 个适用实体（363 个软件/协议类实体不适用）。
换句话说，「这两个零件能不能拧到一起」这个问题，我们目前九成以上答不了。
