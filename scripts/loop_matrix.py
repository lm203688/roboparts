...
LOOPS = [
    ...
    },
    {
        "id": "M2",
        "name": "状态刷新",
        "script": "closure_status.py",
        "detects": "Status file stale / regression RED / unfilled stub / git gate",
        "auto_fix": "Regenerate _STATUS.md from git + regression + facts() + NEEDS_USER",
        "feedback": "_STATUS.md (唯一对外接口) refreshed",
        "closed": True,
        "gap": None,
    },
    {
    ...
    },
    {
        ...
        "detects": "URDF facts (joint/link topology) from public repos",
        "auto_fix": "Parse URDF XML -> ops/urdf_candidates.json",
        "feedback": "ops/urdf_candidates.json updated (mech interface omitted by design)",
        "closed": False,
    ...
    },
    {
        ...
        "detects": "Structural validity of ops/seed-bom.json submissions",
        "auto_fix": "Report only (does not write seed-bom.json nor emit declarations)",
        "feedback": "Inbox structural report; evidence judged by build_flywheel_layer.mjs",
        "closed": True,
        "gap": "Needs real submissions with official-domain source_url",
        "workaround": "Submit via ops/seed-bom.json or /api/bom/import",
    },
